...
/** F-keys shown in the mobile dock (desktop shows all). */

const MOBILE_DOCK_F_KEYS = new Set([1, 2, 4, 9]);
...
const Navbar = () => {
	...
	useEffect(() => {
		...
		const canMagnify = window.matchMedia("(hover: hover) and (pointer: fine)").matches;

		if (!canMagnify || prefersReduced) return;
		...
		dock.addEventListener("touchstart", resetIcons, { passive: true });
		...
		return () => {
			...
			dock.removeEventListener("touchstart", resetIcons);
	...
	const dockScrollClass =
		"dock-scroll relative z-10 items-end justify-center gap-1.5 overflow-x-auto overflow-y-visible px-1.5 pb-1.5 pt-1.5 sm:gap-2 sm:overflow-visible sm:px-3.5 sm:pb-3 sm:pt-2.5";



	const renderDesktopDock = () => (

		<div ref={dockRef} className={`${dockScrollClass} dock-scroll--desktop`}>
	...
	const renderMobileDock = () => (
		...
		<div className={`${dockScrollClass} dock-scroll--mobile`}>

			{primaryItems.map((item, i) => {

				const fKey = i + 1;

				if (!MOBILE_DOCK_F_KEYS.has(fKey)) return null;
			...
			<DockActionButton
				...
				onClick={() => toggleChat(mobileChatRef.current)}

				itemRef={mobileChatRef}
	...
	return (
		...
		<nav
			...
			className={`dock-nav fixed inset-x-0 bottom-0 z-[100] flex justify-center overflow-visible px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 sm:px-3 sm:pb-[max(0.65rem,env(safe-area-inset-bottom))] sm:pt-14 ${
