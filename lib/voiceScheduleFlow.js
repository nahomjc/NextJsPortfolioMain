import { phoneForSpeech } from "./phoneForSpeech";
...
/** "a-i-y-a" or "j-m-a-i-l" → collapse hyphenated letter spelling */
function collapseHyphenSpelling(segment) {
	...
	return s
		.split(".")
		.map((label) =>
			/^([a-z0-9]-)+[a-z0-9]$/i.test(label) ? label.replace(/-/g, "") : label,
		)
		.join(".");
}

function fixCommonSttEmailDomains(email) {
	...
	if (bare === "hotmailcom") domain = "hotmail.com";
	if (bare === "outlookcom") domain = "outlook.com";
	if (bare === "yahoocom") domain = "yahoo.com";
	return `${local}@${domain}`;
}

export function normalizeSpokenName(text) {
	return (text || "")
		.trim()
		.replace(/^(my\s+name\s+is|i'?m|i am|this is|it'?s|call me)\s+/i, "")
		.replace(/[.!?]+$/, "")
		.trim();
}
...
function collapseSpelledSegment(segment) {
	...
	if (tokens.length === 1) return collapseHyphenSpelling(tokens[0]);
...
export function normalizeSpokenEmail(text) {
	...
	if (atParts.length < 2) {
		...
		return fixCommonSttEmailDomains(collapseHyphenSpelling(s));
	}
	...
	email = email.replace(/\b(g\s*mail|gee\s*mail|j\s*mail)\b/g, "gmail");
	...
	return fixCommonSttEmailDomains(collapseHyphenSpelling(email));
}
...
function isAffirmative(text) {
	return /^(yes|yeah|yep|sure|ok|okay|confirm|send(\s+it)?(\s+now)?|do it|go ahead|please|correct|that's right|sounds good|right)/i.test(
		text.trim(),
	);
}
...
function normalizeTimeWindow(text) {
	...
	if (/\b(even|eve|open)\b/.test(t)) return "Evening";
...
export function processScheduleInput(session, text) {
	...
	if (session.step === "confirm") {
		if (isAffirmative(trimmed)) {
			if (!isPlausibleEmail(session.data.email)) {
				session.data.email = "";
				session.pendingEmail = "";
				session.step = "email";
				return {
					kind: "reply",
					reply: "That email looks invalid. Let's try your email again.",
				};
			}
	...
	if (session.step === "name") {
		const name = normalizeSpokenName(trimmed);
		if (name.length < 2) {
